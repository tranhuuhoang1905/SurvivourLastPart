...
Shader "Gonbest/Shadertoy/Disney" 
{ 
    Properties{
...
vec3  g_camPointAt   = tvec3(0.);
vec3  g_camOrigin    = tvec3(0.);
...
{
    return true;//all(lessThanEqual(abs(p), .5 * bounds));
}
...
// Returns the ray intersection distance (assumes rd is GBNormalizeSafed) to the 
...
SurfaceData tSurfaceData( vec3 vertex,vec3 normal,vec3 basecolor,float roughness,float specular,float metallic,float selfshad)
{
    ...
    result.selfshad = selfshad;
    return result;

}

#define INITSURF(p, n) tSurfaceData(p, n, tvec3(0.), 0., 1., 0., 0.)
...
BRDFVars tBRDFVars(vec3 vdir, vec3 hdir,float costh,float costd,float costl,float costv)
{
        BRDFVars result= (BRDFVars)0;
        result.vdir = vdir;
        result.hdir = hdir;
        result.costh = costh;
        result.costd = costd;
...
{
    vec3 vdir = GBNormalizeSafe( g_camOrigin - surf.vertex );
    vec3 hdir = GBNormalizeSafe(ldir + vdir);
    ...
    return tBRDFVars(vdir, hdir, costh, costd, costl, costv);
              ...
              inout SurfaceData surf)
{
    ...
    {
        vec4 pavem = tex2D(iChannel2, .1 * surf.vertex.xz);
        surf.basecolor = tvec3(.1 * smoothstep(.6, .3, pavem.r));
        surf.metallic = .0;
        surf.roughness = .6;
        surf.specular = .02; // .05
        
        surf.normal.xz += .1 * pavem.bg;
        surf.normal = GBNormalizeSafe(surf.normal);
...
{
    // F(h,l) factor
    vec3 F0 = surf.specular * mix(tvec3(1.), surf.basecolor, surf.metallic);
...
vec3 integrateDirLight(vec3 ldir, vec3 lcolor, SurfaceData surf)
{
    ...
    if (ndl > 0.)
    {
        ...
        float shd = min(1. - surf.selfshad, calcSoftShadow( surf.vertex, ldir, 0.1, 20., 5.));
...
vec3 sampleEnvLight(vec3 ldir, vec3 lcolor, SurfaceData surf)
{
    ...
    if (ndl > 0.)
    {
        ...
        float shd = min(1. - surf.selfshad, calcSoftShadow( surf.vertex, ldir, 0.02, 20., 7.));
...
{
    vec3 vdir = GBNormalizeSafe( surf.vertex - g_camOrigin );    
    ...
    vec4 specolor = .4 * mix(tex2D(iChannel0, envdir),tex2D(iChannel1, envdir),surf.roughness) * (1. - surf.roughness);
...
{    
    ...
    // shadowing effects.
    float ao = calcAO(surf.vertex, surf.normal);

    vec3 cout   = integrateDirLight(g_ldir, tvec3(g_lillum), surf);
    cout       += integrateEnvLight(surf, tvec3(g_lillum)) * (1. - 3.5 * ao);
    cout       += amb * (1. - 3.5 * ao);
...
void animateGlobals()
{
    ...
    g_camOrigin = mul(rotMatAroundXAxis(cosrotx, sinrotx), g_camOrigin);
    g_camOrigin = mul(rotMatAroundYAxis(cosroty, sinroty),  g_camOrigin);
    ...
    g_ldir = mul(rotMatAroundYAxis(coslroty, sinlroty) , g_ldir);
...
CameraData tCameraData(vec3 origin,vec3 dir,vec2 st)
{
    CameraData result= (CameraData)0;
...
CameraData setupCamera(vec2 fragCoord)
{
    ...
    vec3 iz = GBNormalizeSafe( g_camPointAt - g_camOrigin );
    vec3 ix = GBNormalizeSafe( cross(iz, iu) );
    vec3 iy = cross(ix, iz);

    vec3 dir = GBNormalizeSafe( st.x*ix + st.y*iy + 1.0 * iz );

    return tCameraData(g_camOrigin, dir, st);
...
{   
    ...
    // Gamma correct
    scenecol = pow(scenecol, tvec3(0.45));

    // Contrast adjust - cute trick learned from iq
    scenecol = mix( scenecol, tvec3(dot(scenecol,tvec3(0.333))), -0.6 );
