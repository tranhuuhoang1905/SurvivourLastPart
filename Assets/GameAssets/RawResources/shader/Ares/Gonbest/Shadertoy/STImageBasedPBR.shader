...
Desc:这个Shader只是一个模板,用于创建把Shadertoy上面的代码,扒下来在Unity上进行调试的Shader代码.主要是用于研究Shadertoy上渲染表现的实现方式.
    ...
    4.其他的基本上都已经定义了.
*/
Shader "Gonbest/Shadertoy/STImageBasedPBR" 
{ 
    Properties{
...
//----------------------
// Constants 
#define  GEO_MAX_DIST    50.0
#define  MATERIALID_SKY     2
#define  MATERIALID_SPHERE  3
#define   F_ALU_N   vec3(1.600,0.912,0.695) //(Red ~ 670 nm; Green ~ 540 nm; Blue ~ 475 nm)
#define   F_ALU_K   vec3(8.010,6.500,5.800) //(Red ~ 670 nm; Green ~ 540 nm; Blue ~ 475 nm)
...
vec3 PBR_Equation(vec3 V, vec3 L, vec3 N, float roughness,  vec3 ior_n,   vec3 ior_k,   bool metallic,  bool bIBL)
{
    float cosT = saturate( dot(L, N) );
    float sinT = sqrt( 1.0 - cosT * cosT);
	vec3 H = GBNormalizeSafe(L+V);
...
{
    vec3 linearGammaColor_sharp = PBR_HDRremap(pow(texture( iChannel2, sampleDir ).rgb,tvec3(2.2)));
    vec3 linearGammaColor_blur  = PBR_HDRremap(pow(texture( iChannel3, sampleDir ).rgb,tvec3(1)));
    vec3 linearGammaColor = mix(linearGammaColor_sharp,linearGammaColor_blur,saturate(LOD_01));
    return linearGammaColor;
}

//Arbitrary axis rotation (around u, GBNormalizeSafed)
...
vec3 PBR_importanceSampling(vec3 sampleDir, float roughness, float e1, float e2, out float range)
{
    ...
    vec3 othogonalAxis = GBNormalizeSafe(cross(notColinear,sampleDir));
	mat3 m1 = PBR_axisRotationMatrix(GBNormalizeSafe(othogonalAxis), cos(range), sin(range));
	mat3 m2 = PBR_axisRotationMatrix(GBNormalizeSafe(sampleDir),     cos(phi),   sin(phi));
	return mul(mul(m1,m2),sampleDir);
}

vec3 PBR_visitSamples(vec3 V, vec3 N, float roughness, bool metallic, vec3 ior_n, vec3 ior_k )
{
    #define  MIPMAP_SWITCH  0.29 //sampling angle delta (rad) equivalent to the lowest LOD.
    #define SAMPLE_COUNT  ivec2(05,15) //(5 random, 15 fixed) samples
    #define  weight vec2(1./float(SAMPLE_COUNT.x),1./float(SAMPLE_COUNT.y))
...
vec4 MAT_triplanarTexturing(vec3 p, vec3 n)
{
    ...
    vec3 stitchingFade = tvec3(1.)-smoothstep(tvec3(0.5-sw),tvec3(0.5),abs(p-0.5));
...
TraceData tTraceData(float rayLen,vec3  rayDir,vec3  normal,int   matID)
{
    TraceData ret= (TraceData)0;
    ret.rayLen = rayLen;
    ret.rayDir = rayDir;
    ret.normal = normal;
    ret.matID = matID;
...
{
    //Roughness texture
    vec4 roughnessBuffer = tvec4(0.5);//MAT_triplanarTexturing(pos*1.5,traceData.normal);
    ...
    vec3 V = GBNormalizeSafe(-traceData.rayDir);
    vec3 N = traceData.normal;
    vec3 L = GBNormalizeSafe(vec3(1,1,0));
    ...
    col     += PBR_HDRremap(tvec3(1))*L0*ABL_LIGHT_CONTRIBUTION;
    
    //Anti-aliasing trick (normal-based)
    vec3 backgroundColor = pow(texture( iChannel2, traceData.rayDir ).xyz,tvec3(2.2));
...
TraceData TRACE_geometry(vec3 o, vec3 d)
{
    ...
    return tTraceData(t,d,GBNormalizeSafe(dfHitPosition),bBackground?MATERIALID_SKY:MATERIALID_SPHERE);
}
...
{
    //Camera & setup
    vec4 cSlider = tvec4(1);//processSliders(fragCoord);
    ...
    vec3 camD = GBNormalizeSafe(tvec3(0)-camO);
    vec3 camR = GBNormalizeSafe(cross(camD,vec3(0,1,0)));
    vec3 camU = cross(camR,camD);
   	vec3 dir =  GBNormalizeSafe(uv.x*camR+uv.y*camU+camD);
    
    //Raytrace
    TraceData geometryTraceData = TRACE_geometry(camO, dir);
    vec3 ptGeo = (geometryTraceData.rayLen < GEO_MAX_DIST)? camO+dir*geometryTraceData.rayLen : tvec3(0);
    ...
    c = pow(c*vignetting,tvec3(0.4545)); //2.2 Gamma compensation
