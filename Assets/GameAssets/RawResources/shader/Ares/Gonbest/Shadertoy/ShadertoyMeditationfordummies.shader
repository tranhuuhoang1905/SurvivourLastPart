...
Desc:这个Shader只是一个模板,用于创建把Shadertoy上面的代码,扒下来在Unity上进行调试的Shader代码.主要是用于研究Shadertoy上渲染表现的实现方式.
    ...
    4.其他的基本上都已经定义了.
*/
Shader "Gonbest/Shadertoy/Meditationfordummies" 
{ 
    Properties{
   ...
   // Created by sebastien durand - 2015
    ...
    #define g_traceSize .005

    // globals
    #define g_nozePos   vec3(0,-.28+.04,.47+.08)
    #define g_eyePos   vec3(.14,-.14,.29)
    #define g_eyeSize  .09
    ...
    float noise( in vec3 x ) {
        ...
        vec2 rg = textureLod( iChannel0, vec4((uv+ 0.5)/256.0, 0.0 , 0) ).yx;
    ...
    float dEar(in vec3 p, in float scale_ear) {
        vec3 p_ear = p*scale_ear;
        p_ear.xy = mul(ma,p_ear.xy);
        p_ear.xz = mul(ma,p_ear.xz);
        float d = max(-sdEllipsoid(p_ear-vec3(.005,.025,.02), vec3(.07,.11,.07)), 
                        sdEllipsoid(p_ear, vec3(.08,.12,.09)));

        p_ear.yz = mul(ma,p_ear.yz);
        d = max(p_ear.z, d); 
    ...
    // Eyelid ---------------------------------
        ...
        //p_eye1.yz *= g_eyeRot;
        p_eye1.yz = mul(g_eyeRot,p_eye1.yz);
        //p_eye2.zy *= mc;
        p_eye2.zy = mul(mc,p_eye2.zy);
    ...
    float map( vec3 p) {
        ...
        p -= headRotCenter;
        //p.yz *= g_headRotH;
        p.yz = mul(g_headRotH,p.yz);

        //p.xz *= g_headRot;
        p.xz = mul(g_headRot,p.xz);
        p += headRotCenter;
    ...
    vec3 Sky( vec3 ray) {
        return g_envBrightness*mix( tvec3(.8), tvec3(0), exp2(-(1.0/max(ray.y,.01))*vec3(.4,.6,1.0)) );
    }
    ...
    vec3 iris(vec2 p, float open)
    {
        ...
        if (r< .8) {
            ...
            a += .05 * fbm(20.*p);
            col = mix(col, tvec3(1), smoothstep(.3, 1., fbm(vec2(5. * r, 20. * a))));  // white highlight
            ...
            col = .5*mix(col, tvec3(1), smoothstep(.75, .8, r));
        }
    ...
    vec3 Shade( vec3 pos, vec3 ray, vec3 normal, vec3 lightDir1, vec3 lightDir2, vec3 lightCol1, vec3 lightCol2, float shadowMask1, float shadowMask2, float distance )
    {
        ...
        vec3 ambient = g_envBrightness*mix( vec3(.2,.27,.4), tvec3(.4), (-normal.y*.5+.5) ); // ambient
        ...
        float specularity = .2; 
        vec3 h1 = GBNormalizeSafe(lightDir1-ray);
        vec3 h2 = GBNormalizeSafe(lightDir2-ray);
        ...
        if (colorId < .5) {  
            ...
            if (p.z>0.) {
                ...
                g_lightPos2.yz = mul(g_headRotH, g_lightPos2.yz);
                //g_lightPos2.xz *= g_headRot;
                g_lightPos2.xz = mul(g_headRot, g_lightPos2.xz);
                g_lightPos2 += headRotCenter;

                vec3 dir = GBNormalizeSafe(g_lightPos2-g_eyePosloc);
                
                float a = clamp(atan2(-dir.x, dir.z), -.6,.6), 
                    ca = cos(a), sa = sin(a);
                //pe.xz *= mat2(ca, sa, -sa, ca);
                pe.xz = mul(mat2(ca, sa, -sa, ca),pe.xz);
                

                float b = clamp(atan2(-dir.y, dir.z), -.3,.3), 
                    cb = cos(b), sb = sin(b);
                //pe.yz *= mat2(cb, sb, -sb, cb);
                pe.yz = mul(mat2(cb, sb, -sb, cb),pe.yz);
                
                albedo = (pe.z>0.) ? iris(17.*(pe.xy), length(g_lightPos2-g_eyePosloc)) : tvec3(1);
            }
            specPower *= specPower;
        }
    ...
    #ifndef DRAW_DISTANCE       
        bool g_bHead = intersectSphere(pos, ray, vec3(0,-.017,.02), .65, t0, t1);
        bool g_bBody = intersectSphere(pos, ray, vec3(0,-.7,.02), .5, t2, t3);
        if (g_bHead || g_bBody) 
    #endif        
        {   
    ...
    //#endif
        ...
        //return GBNormalizeSafe(grad);
        // prevent normals pointing away from camera (caused by precision errors)
        return GBNormalizeSafe(grad - max(.0,dot (grad,ray ))*ray);
    }
    ...
    #define a_eyeClose  .55
    #define a_eyeOpen  -.3


    
    #define t_apear  5.
    #define t_noze  t_apear+8. 
    #define t_openEye  t_noze + 1.
    #define t_g_headRot  t_openEye + 4.5
    #define t_rotDown  t_g_headRot + 3.5
    #define t_outNoze  t_rotDown + 3.
    #define t_night  t_outNoze + 4.
    #define t_colorfull  t_night + 5.
    #define t_disapear  t_colorfull + 2.
    #define t_closeEye  t_disapear + 3.
    ...
    // Position du nez
        ...
        p_noze.xz = mul(g_headRot2,p_noze.xz);
        //p_noze.yz *= g_headRotH2;
        p_noze.yz = mul(g_headRotH2,p_noze.yz);
    ...
    // intensitee et couleur du point
        ...
        vec3 lightDir1 = GBNormalizeSafe(vec3(.5,1.5,1.5));
        ...
        for (int i=0;i<ANTIALIASING;i++) {
            ...
            {           
                ...
                // fog
                float f = 200.0;
                col = mix( tvec3(.8), col, exp2(-t*vec3(.4,.6,1.0)/f) );
            }
            else
            {
                col = Sky( ray );
        ...
        float l = max(col.x,max(col.y,col.z));//dot(col,GBNormalizeSafe(vec3(2,4,1)));
        l = max(l,.01); // prevent div by zero, darker colours will have no curve
        float l2 = SmoothMax(l,1.0,.01);
        col *= l2/l;
        
        fragColor =  vec4(pow(col,tvec3(1./1.6)),1);
    }
